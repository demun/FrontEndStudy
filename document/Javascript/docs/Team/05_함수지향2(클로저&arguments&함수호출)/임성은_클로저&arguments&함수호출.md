...
```
for문이 돌면서 위에서 강조 표시하고 있는 함수로직이 실행이 될것이고, id라고 하는 값은 매개변수 즉, 지역변수로서의 역할을 하고 있다. i의 값을 받아서 함수 내부로 전달하고 잇다. 또한 전달과 동시에 리턴을 하고 있는데, 내부함수를 리턴을 하고 있고 내부함수는 외부함수의 지역변수 id를 사용하고 있다.
===> 실행결과 : 0,1,2,3,4
