...
<script>
  ...
  function cal(mode) {
     var funcs = {
        ...
        'minus' : function(left,  fight) {return left - right}
     }
  ...
  var sortfunc = function(a, b){  //인자로 전달하는 함수 비교값 수행
     return a - b;
  }
  console.log(numbers.sort(sortfunc));  //sortfunc콜백함수가 되는것이다.
  ===>실행결과: 1,2,3,6,8,10,20
</script>
```
...
<script>
  function sum(){     //매개변수가 없는 sum()이란 함수
   ...
   document.write('result : ' + sum(1,2,3,4));  //매개변수가 없는데도 sum이란함수에 1,2,3,4 인자4개를 전달하고 있다.
